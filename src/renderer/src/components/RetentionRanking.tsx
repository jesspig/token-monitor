      ...
      <div className="divide-y divide-neutral-800/60">
        {sorted.map((row, idx) => (
          <div key={row.model} className="grid grid-cols-12 items-center gap-2 px-3 py-2.5 text-xs">
            <div className="col-span-1 tabular-nums text-neutral-500">{String(idx + 1).padStart(2, '0')}</div>
            ...
            <div className="col-span-2 text-right tabular-nums text-neutral-200">{formatPercent(row.rate)}</div>
            <div className="col-span-1 hidden text-right tabular-nums text-neutral-500 sm:block">{row.eligible.toLocaleString()}</div>
          </div>
        ))}
      </div>
    </div>
