...
- **首次**：全量扫描 → 游标记录行数（opencode db 源及其同构 fork dev-eco/mimo 为 `time_created` 水位，goose/minimax 为 rowid 水位）。第三批源还复用了两类非行号游标：**数组水位**（trae-agent，fromLine = `llm_interactions[]` 已导入条数，条目数回退即全量重析）与**快照水位**（codewhale 会话级 `total_tokens` 快照差分；copilot-cli per-file delta 状态机对累计快照取增量，详见 [监控插件](monitor-plugins.md)）。
...
- **外部 SQLite 只读连接 busy 短超时**：opencode / zcode / zed 插件打开上游 SQLite 库统一传 `EXTERNAL_DB_BUSY_TIMEOUT_MS = 250`（zed 为文件内常量），qoder / qoder-cn 经 `_lib/qoder-shared.ts` 传 `QODER_DB_BUSY_TIMEOUT_MS = 250`，dev-eco / mimo 经 `_lib/opencode-shared.ts` 传 `OPENCODE_LIKE_DB_BUSY_TIMEOUT_MS = 250`，goose / minimax 插件文件内常量 `BUSY_TIMEOUT_MS = 250`——better-sqlite3 撞锁时在主线程同步忙等，默认 5000ms 会冻结整个应用；现撞锁约 250ms 即放弃本轮（返回空结果），由下轮兜底重试。
...
2. **语义去重（fork/rewrite 场景）**：31 插件中 25 个产出稳定 `source.requestId`（第二批 14 源中 12 源产出、第三批 9 源中 5 源产出；kimi / reasonix / goose / gptme / trae-agent / minimax 六源例外，退回主键去重）——claude=`message.id`/`uuid`、codex=`threadId:行timestamp:in-cached-out` / stream_error 无 id（退回主键）、opencode 及其同构 fork（dev-eco/mimo）=`db 行主键 id`（`SELECT m.*` 兼容 `message.id` 回退）、gemini=消息 `id`、grok=`sid:loop_index` 全量重析、pi/zcode/dsh、第二/三批各源的逐源规则见 [监控插件](monitor-plugins.md)。`recordUsage` 事务内按 `(data_source, request_id)` 查 `dedup_ledger`（`SELECT ... WHERE data_source=? AND request_id=?`），命中即跳过（不入明细、不计 rollup、不计 addedRecords、不触发 usage-updated）；插入成功回填账本 `semantic_id=sha256 前 16 位`（仅存证，判定按 `request_id` 直配）；失败记录常零 token，其 `semantic_id` 可能相同但**不参与判定**，主路径仍以 `(data_source, request_id)` 为准，不会误合并。无 requestId 的记录退回主键去重。
