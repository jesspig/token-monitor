...
type: sync-design
title: 同步与去重
description: 增量游标 + chokidar 监听 + 定时兜底扫描；主键幂等去重 + fork/rewrite 语义去重账本（requestId 直配）；清理前预回填。
...
> [!note] 当前状态
> **已实现**（2026-08-20）。游标/去重落地于 `storage.ts`，调度与监听于 `scheduler.ts`/`watcher.ts`，采集编排于 `collector.ts`；语义去重账本接入、opencode WAL mtime 感知与清理前预回填于 2026-08-23 落地。
