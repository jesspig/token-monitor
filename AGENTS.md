...
| `$env:ELECTRON_RUN_AS_NODE='1'; & "node_modules\electron\dist\electron.exe" node_modules/vitest/vitest.mjs run` | **全量单测（1061 用例）**。`package.json` 无 test 脚本；better-sqlite3 已编为 Electron ABI，系统 Node 跑 vitest 报 `NODE_MODULE_VERSION` 错，**只能这样跑** |
...
- 数据流：`CLI 会话文件 → 插件增量解析(行游标) → 去重 → 费用计算 → usage_records → usage_daily_rollups（日聚合，永不清理）/ usage_hourly_rollups（小时聚合，v10）→ 前端查询`；`getDailyModelBreakdown` 无维度过滤时走 `usage_daily_rollups` 预聚合快路径，否则回退明细表（`idx_usage_records_model_created` 兜底，v11）；v13 重建缓存口径十源部分索引（`idx_usage_records_cached_input`）。
- 插件框架（`src/main/core/`：registry/context/lifecycle/event-bus）：**新增监控对象 = 新增 `plugins/<id>.ts` 实现 `MonitorPlugin`，并在 `host.ts` 的 `BUILTIN_PLUGINS` 登记一行**；经服务容器 `ctx`（storage/pricing/events/scheduler/watcher）访问能力，不直接 import 宿主实现。当前 31 插件：claude/codex/opencode/gemini/grok/pi/zcode/dsh + workbuddy/codebuddy/cline/roo-code/kilo-code/qwen/qoder/qoder-cn/kimi/zed/kiro/reasonix/command-code/copilot-chat + dev-eco/mimo/goose/copilot-cli/gptme/trae-agent/codewhale/droid/minimax。
...
- 计费语义：`input_semantics` 三态（0=未知 / 1=含缓存总量需扣减 / 2=纯新输入）；semantics=1：codex/gemini/grok/zcode/workbuddy/codebuddy/qwen/reasonix/goose/copilot-cli（10 源）；semantics=0：qoder/qoder-cn/kiro/codewhale（4 源，codewhale 为会话级总量快照、分项缺失）；其余 claude/opencode/pi/dsh/cline/roo-code/kilo-code/kimi/zed/command-code/copilot-chat/dev-eco/mimo/gptme/droid/minimax 为 2（16 源）；trae-agent 行级：openai 系 provider=1 / anthropic 系=2；`calcCost` 对 semantics=1 先扣缓存再乘价。
- 去重：记录 id = `data_source:file_path:line` + `INSERT OR IGNORE`；31 插件中 25 个产出稳定 `source.requestId`（kimi/reasonix/goose/gptme/trae-agent/minimax 例外，依赖行主键幂等），入库事务内按 `(data_source, request_id)` 查写 `dedup_ledger` 做 fork/rewrite 语义去重。
