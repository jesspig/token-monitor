...
## 知识库（docs/）
- 位置 `docs/`：项目唯一设计依据。结构 `index.md`（目录）/ `log.md`（按天摘要，仅留 7 天）/ `concepts/*.md`（概念页，当前 10 页：overview/architecture/data-flow/data-model/sync-mechanism/monitor-plugins/plugin-architecture/ui-pages/pricing/roadmap；YAML frontmatter：`type` 必填、`resource` 指向源码、`timestamp` 真实时间）/ `changelog/`（按小时记录）。
